...
const { learnFromSuccess } = require('../ENGINE_CORE/intelligence/learning_engine.cjs');
...
function findBestSkill(description) {
    const goldminePath = getPath('INTELLIGENCE', 'GOLDMINE_DIR') ? path.join(getPath('INTELLIGENCE', 'GOLDMINE_DIR'), 'EXTRACTED', 'plugins') : null;
    if (!goldminePath || !fs.existsSync(goldminePath)) return "SKILL-NOT-FOUND";
...
async function finalizeMission(agentKey, fileName, content, sourcePath) {
    ...
    try {
        ...
        const ledgerEntry = `\n> [!NOTE]\n> **RÉSULTAT MISSION** : ${missionId}\n` +
                            ...
                            `---`;
        const coreLedgerPath = getPath('TECH', 'COCKPIT');
        if (coreLedgerPath && fs.existsSync(coreLedgerPath)) {
            fs.appendFileSync(coreLedgerPath, ledgerEntry);
        }
